define(["comp/common/collapse",
    ...
    var handleTrain = function(){
        ...
        $.get( modelCreateUrl, para, function( data ) {
            if ( data.result === "Success") {
                ...
                //trainPara.features = dataBinding.Features.join();mainpoint
    ...
    var updateTrainResult = function() {
        var props = {};
        props.cols = (dataBinding.mainpoint.slice(0)+","+( dataBinding.otherpoint.slice(0))).split(',');  // Clone the array
    ...
    var updateVizPanel = function() {
        ...
        $.get(predictVizUrl, param, function( data ) {
            if (data.result === "Success" ) {
                ...
                option.features = dataBinding.otherpoint;
                option.target = dataBinding.mainpoint;
                ...
                <div className="row">
                    <div className="col-md-4">
                        <div className="panel-group" role="tablist" aria-multiselectable="true">
                            ...
                            <Collapse title="选择模型" id="SelectModelCollapse">
                                <select className="input-medium" id="selectModelType"/>
                            </Collapse>

                            <Collapse title="选择数据文件" id="SelectDataCollapse">
                                <DataSelection handleSelection={handleDataSelection} />
                            </Collapse>

                            <Collapse title="选择训练数据" id="SelectBindingCollapse">
                                <div id="mlBindingPanel"/>
                            </Collapse>

                            <Collapse title="训练模型" id="TrainCollapse">
                                <div>
                                    <button className="btn btn-default btn-xs" onClick={handleTrain}>训练</button>
                                </div>
                            </Collapse>
                        </div>
                    </div>
                    <div className="col-md-8">
                        <Panel id="MLMain" title="预测展示">
                            <Tab data={tabOption} />
                        </Panel>
                    </div>
                </div>

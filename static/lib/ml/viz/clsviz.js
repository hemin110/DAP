...
define([], function () {
    ...
    var CLSViz = function CLSViz(option) {
        ...
        this._svg = d3.select("#" + this._rootContainerId).append("svg").attr("width", this._width + margin.left + margin.right).attr("height", this._height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    };
    ...
    CLSViz.prototype.render = function () {
        ...
        this._svg.append("g").attr("class", "predict").selectAll(".area").data(this._predictData).enter().append("rect").attr("class", "area").attr("x", function (d) {
            return x(d.x);
        }).attr("y", function (d) {
            return y(d.y);
        }).attr("width", xStep).attr("height", yStep).style("fill", function (d) {
            return color(d.label);
        }).style("fill-opacity", 0.3);

        var legend = this._svg.selectAll(".legend").data(color.domain()).enter().append("g").attr("class", "legend").attr("transform", function (d, i) {
            return "translate(" + (i * 100 - 400) + ", -30)";
        });

        legend.append("rect").attr("x", this._width - 18).attr("width", 18).attr("height", 18).style("fill", color);

        legend.append("text").attr("x", this._width - 24).attr("y", 9).attr("dy", ".35em").style("text-anchor", "end").text(function (d) {
            return d;
        });
    };

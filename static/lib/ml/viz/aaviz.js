...
define([], function () {
	...
	var AAViz = function AAViz(option) {
		...
		this._svg = d3.select("#" + this._rootContainerId).append("svg").attr("width", 450).attr("height", 450).append("g").attr("transform", "translate(" + 450 / 2 + "," + 450 / 2 + ")");
	};
	...
	AAViz.prototype.render = function () {
		...
		var popudata1 = [[90, 81, 36], [81, 100, 11], [36, 11, 99]];
		console.log(popudata1);
		var chord_layout = d3.layout.chord().padding(0.03).sortSubgroups(d3.descending()).matrix(popudata);
		...
		g_outer.selectAll("path").data(groups).enter().append("path").style("fill", function (d) {
		...
		this._svg.append("g").attr("class", "chord").selectAll("path").data(chords).enter().append("path").attr("d", inner_chord).style("fill", function (d) {
			return color(d.source.index);
		}).style("opacity", 1).on("mouseover", function (d, i) {
			d3.select(this).style("fill", "yellow");
		}).on("mouseout", function (d, i) {
			d3.select(this).transition().duration(1000).style("fill", color(d.source.index));
		});
	};
	return AAViz;
});